...
carousel.prototype.playDot = function () {
  this.$dot.children().removeClass('active').eq(this.curPageIndex).addClass('active' //链式调用
  );
};
...
tabSwitch.prototype.tabChange = function () {
  this.$tab.children().removeClass('on').eq(this.curIndex).addClass('on' //链式调用
  );
};

var TabSwitch = exports.TabSwitch = function () {
  return {
    init: function init($nodeList) {
      $.each($nodeList, function (index, node) {
        new tabSwitch($(node));
